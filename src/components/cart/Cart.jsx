...
import {deleteAllPizza} from "../../redux/cartReducer";

const Cart =()=> {
    const dispatch = useDispatch()
    const {totalPrice, totalCount, cartList} = useSelector(({cartInfo}) => cartInfo)
    const cartPizzasList = Object.keys(cartList).map(key=>{
        return cartList[key][0]
    })
    const cartItems = cartPizzasList.map((item,index) => <CartItem item = {item} index = {index}/>)
                ...
                <div className="cart">
                    <div className="cart__top">
                        ...
                        <div className="cart__clear">
                            ...
                            <span onClick={onDeleteAllClick}>Очистить корзину</span>
                        </div>
                    </div>
